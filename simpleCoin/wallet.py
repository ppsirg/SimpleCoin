...
def generate_ECDSA_keys():
    ...
    """
    filename = input("Write the name of your new address: ") + ".key"
    while True:
...
def send_transaction_ui():
    """Send transaction user interface.
    """
    key_filename = input("From: introduce your keys name\n")
    with open(key_filename + '.key', 'r') as f:
        key_data = json.load(f)
    addr_from = key_data['public_key']
    private_key = key_data['private_key']
    print('your key {} was loaded'.format(key_filename))
    print(key_data)
